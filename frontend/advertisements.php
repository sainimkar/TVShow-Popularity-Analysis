        ...
        <table class="table table-striped table-hover">
            <thread>
                <tr>
                    <th>Id</th>
                    <th>space name</th>
                    <th>space allotted</th>
                    <th>show allotted</th>
                </tr>
            </thread>
            <tbody>
            <?php
            $query = "SELECT spaceId,spaceName,spaceAllotted,show_allotted from advertisements";
            $result = mysqli_query($conn,$query)  or die(mysql_error());

            if ($result->num_rows > 0) {

                while($row = $result->fetch_assoc()) {
                    echo "<tr> <td>" . $row["spaceId"]. "</td> 
                       <td>" . $row["spaceName"] . "</td>
                       <td>" . $row["spaceAllotted"]. "</td>
                       <td>" . $row["show_allotted"]. "</td>
                       </tr>";
                }
                echo "</table>";
            }
