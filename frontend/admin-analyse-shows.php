...
<div id="right-panel" class="right-panel">
    ...
    <table class="table table-striped table-hover">
        ...
        <tbody>
    <?php
        $query = "SELECT show_id, show_name,show_genre from shows";
        $result = mysqli_query($conn,$query)  or die(mysql_error());

        if ($result->num_rows > 0) {

        while($row = $result->fetch_assoc()) {
            echo "<tr> <td>" . $row["show_id"]. "</td> 
                       <td>" . $row["show_name"] . "</td>
                       <td>" . $row["show_genre"]. "</td>
                       <td><a class='btn btn-primary btn-lg'  href='admin-ind-show-analyse.php?show_id=".$row['show_id']."'>Analyse</a></td>
                       </tr>";
        }
            echo "</table>";
    }
